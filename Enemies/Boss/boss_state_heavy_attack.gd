class_name BossStateHeavyAttack
extends EnemyState

@onready var chase: EnemyState = $"../Chase"

@export var total_duration: float = 1.5
@export var hit_start_frame: int = 17
@export var hit_end_frame: int = 23
...
func Enter() -> void:
	...
	boss.mark_attack_used("heavy")
	boss.use_attack_box("heavy")

	timer = 0.0
	active = false

	enemy.reset_attack_window()
	enemy.updateAnimation("heavy")
